...
### 方式一：build.sh（PyInstaller + py2pyd 加密）
...
打包流程：
1. PyInstaller `--onedir` 打包，`app_edgeStress` 以文件夹形式保留在 `_internal/`
2. 删除旧版 UI 代码
3. `py2pyd` 将业务 `.py` 编译为 `.so`，删除源码

产物目录：`dist_backend/`，运行：`./dist_backend/edge_stress_backend`

### 方式二：auto-py-to-exe（GUI 方式）

使用 auto-py-to-exe 打包，产物目录：`output/main/`

### 嵌入 Electron 前端

打包完成后，将产物复制到前端的 `resources/backend/` 目录：

```bash
cp -R output/main/* ../frontend/resources/backend/
```

Electron 主进程会在启动时自动运行后端，关闭时自动停止。
